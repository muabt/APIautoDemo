...
Feature: Create new data model
  ...
  Scenario: Create new data space
    ...
    And I access table "Dataspaces/snapshots"
    And I select filter by text with keyword and field below
      | Field contains: | In fields |
      | Store           |           |
    And I delete the dataspace "Dataspace Store (Store)" with service "Actions>Delete dataspaces and snapshots recursively"
