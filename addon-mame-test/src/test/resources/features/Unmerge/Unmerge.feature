...
Feature: Run Unmerge service
  ...
  Scenario: SC-uM14 Check Metadata & Value of records in group after running Unmerge successfully for Golden was an auto-created record and there is not any interpolation merged (MI) record target to this Golden
    When I access table "Audience" of dataset "Unmerge Service" in dataspace "Enhance_Unmerge_Service"
    And I select filter by advanced search with criterion and logical "At least one criterion matches"
    ...
    And I access table "RecordMetadata" of dataset "Unmerge_Service_Audience_MDS" in dataspace "Enhance_Unmerge_Service"
    ...
    Then I will see table MergingProcess after changing as below
      | id   | mergePolicyId | mergeMode | executionDate | snapshotId                      | groupId               | user  | isUnmerged |
      | 1,212 | 173           | Manual    | TODAY         | V1566548970216_BUnmerge_Service | 1,609,640,740,111,360 | admin | Yes        |
